class Arco():
    ...
    def oposto(self, city):
        if city!=self.vertices[0]:
...
class Grafo():
    ...
    def calcular_melhores_caminhos_partindo_de(self, city):
        caminho = {}
        Arcoslist = []
        visitados = [city]      
        caminho[city] = (0, [city])
        for x in range(len(self.arco)):
            if city in self.arco[x].vertices:
